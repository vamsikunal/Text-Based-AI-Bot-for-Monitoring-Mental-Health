...
class PlotDiar:
    """
    A viewer of diraization
    """
    def __init__(self, diar, title=''):
        """
        ...
        self.map = self.diar.make_index(['show', 'cluster'])
        #self._draw()

    def draw(self, ax=None):
        """
        Draw the diarization
        ...
        for show in self.map:
            for cluster in sorted(self.map[show].keys()):
                ...
                for row in self.map[show][cluster]:
                    ...
                    if x < minx:
                        minx = x
                    maxx = max(maxx, row['stop'] / 100)
                    c = self.rect_color
                    if row._attributes.exist('color'):
                        c = row['color']
                        #print("**", c)
                    rect = Rectangle((x, y), w, self.height, color=c)
                    ax.add_patch(rect)
                y += self.height
            #print([minx, maxx])
        maxy = y
        ax.set_yticks(labels_pos) # values
        ...
        for show in self.map:
            for cluster in self.map[show]:
                for row in self.map[show][cluster]:
                    x = row['start'] / 100
                    ax.plot([x, x], [0, maxy], linestyle=':',
                                 color='#AAAAAA')
        
#        y = maxy
#        for show in self.map:
#            for cluster in self.map[show]:
#                ax.plot([0, maxx], [y, y], linestyle=':',
#                             color='#AAAAAA')
#                y -= self.height
        ...
        for show in self.map:
            for cluster in sorted(self.map[show].keys()):
                ...
                for row in self.map[show][cluster]:
                    ...
                    if row._attributes.exist('color'):
                        c = row['color']
                    rect = Rectangle((x, y), w, self.height,
