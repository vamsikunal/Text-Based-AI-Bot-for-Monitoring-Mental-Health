...
def adjustBoundAccordingToDiarAndTolerance(diar, diarBasis, diarUem=None, tolerance=25):
    assert isinstance(diar, Diar) and isinstance(diarBasis, Diar) and ((isinstance(diarUem, Diar) and len(diarOverlapArea(diarUem)) == 0)
                                                                       or diarUem is None) and isinstance(tolerance, numbers.Number)
...
def automatonAssignment(diarHyp, diarRef, diarUem=None, tolerance=0, diarFinal__clusterToDeleteAccordingToDiarRef=list()):
    assert isinstance(diarHyp, Diar) and (diarUem is None or isinstance(diarUem, Diar)) and isinstance(diarRef, Diar) and isinstance(
        tolerance, numbers.Number
    ) and isinstance(diarFinal__clusterToDeleteAccordingToDiarRef, list)
    ...
    assert len(
        diarOverlapArea(diarRef)
    ) == 0, "Error: diarRef parameter have some overlapped segments.\nReason: No overlap segment allowed.\nSolution: Please put them apart.\n"
    assert len(
        diarOverlapArea(diarHyp)
    ) == 0, "Error: diarHyp parameter have some overlapped segments.\nReason: No overlap segment allowed.\nSolution: Please put them apart.\n"
...
def automatonSegmentation(
    diarHyp,
    diarRef,
    diarUem=None,
    tolerance=0,
    modeNoGap=False,
    modeNoGap__mergeStrat_BiggestCluster=False,
    diarFinal__clusterToDeleteAccordingToDiarRef=list(),
    deleteBoundarySameConsecutiveSpk=False
):
    assert isinstance(diarHyp, Diar) and isinstance(diarRef, Diar) and isinstance(modeNoGap__mergeStrat_BiggestCluster, bool) and isinstance(
        modeNoGap, bool
    ) and (diarUem is None
           or isinstance(diarUem, Diar)) and isinstance(tolerance, numbers.Number) and isinstance(diarFinal__clusterToDeleteAccordingToDiarRef,
        ...
        if i != 0 and diarRef[i - 1]['stop'] != valueRef['start']:
            ...
            for y in diarHyp:
                if valueRef['start'] - diarRef[i - 1]['stop'] <= tolerance * 2:
                    ...
                    if y['start'] >= (valueRefPrev['stop'] -
                                      tolerance) and y['start'] < (valueRef['start'] -
                                                                   tolerance) and y['stop'] <= (valueRef['start'] +
                                                                                                tolerance) and y['stop'] > (valueRefPrev['stop'] + tolerance):
                        if modeNoGap == False:
                            actionsSegmentationSegmentDelete.append(copy.deepcopy(y))
                            actionsIncrementalSegmentationSegmentDeleteTurn.append(copy.deepcopy(y))
                        valueTmp = dropSegment(y, valueTmp)
                    elif y['start'] >= (valueRefPrev['stop'] - tolerance) and y['start'] < (valueRef['start'] -
                                                                                            tolerance) and y['stop'] > (valueRef['start'] + tolerance):
                        actionsSegmentationBoundaryCreate.append(copy.deepcopy([y, valueRef['start']]))
                        actionsIncrementalSegmentationBoundaryCreateTurn.append(copy.deepcopy([y, valueRef['start']]))
                        valueTmp = splitSegment(y, valueTmp, valueRef['start'])
                        yTmp = copy.deepcopy(y)
        ...
        if len(listHypRefSegment) == 0 or (len(listHypRefSegment) == len(listHypRefSegmentWithinTolerance)):
            ...
            if modeNoGap == False:
                ...
                if segmentExistAccordingToTolerance(valueRef, tolerance):
                    ...
                    actionsSegmentationSegmentCreate.append(
                        copy.deepcopy(
                            Segment([valueRef['show'], valueRef['cluster'], 'speakerManualNotDetected' + str(cpt + 1), valueRef['start'], valueRef['stop']],
                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                        )
                    )
                    actionsIncrementalSegmentationSegmentCreateTurn.append(
                        copy.deepcopy(
                            Segment([valueRef['show'], valueRef['cluster'], 'speakerManualNotDetected' + str(cpt + 1), valueRef['start'], valueRef['stop']],
                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                        )
                ...
                if not perfectBoundary:
                        ...
                        for idx, z in enumerate(listHypRefSegment):
                            ...
                            if boundStop != valueRef['stop']:
                                for r in range(idx + 1, len(listHypRefSegment)):
                                    if (idx != 0 and
                                        z['stop'] <= boundStop) or (z['stop'] >= listHypRefSegment[r]['start'] and z['stop'] <= listHypRefSegment[r]['stop']):
                                        nearStop = None
                                        break
                            ...
                            if nearStop is not None and boundStop != valueRef['stop']:
                                if idx == 0 and z['start'] > valueRef['start'] and valueBoundaryStart != z['start']:
                                    actionsSegmentationSegmentCreate.append(
                                        copy.deepcopy(
                                            Segment([valueRef['show'], clusterName, z['cluster_type'], valueBoundaryStart, z['start']],
                                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                                        )
                                    )
                                    actionsSegmentationSegmentCreate.append(
                                        copy.deepcopy(
                                            Segment([valueRef['show'], clusterName, z['cluster_type'], z['stop'], nearStop],
                                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                                        )
                                    )
                                    actionsIncrementalSegmentationSegmentCreateTurn.append(
                                    ...
                                    valueTmp.append(
                                        show=showname, cluster=clusterName, cluster_type=z['cluster_type'], start=valueBoundaryStart, stop=z['start']
                                    )
                    ...
                    if modeNoGap == False:
                        ...
                        for y in listHypRefSegment:
                            if y['cluster'] != clusterName:
                                ...
                                actionsSegmentationSegmentCreate.append(
                                    copy.deepcopy(
                                        Segment([valueRef['show'], yTmp['cluster'], yTmp['cluster_type'], yTmp['start'], yTmp['stop']],
                                                ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                                    )
                                )
                                actionsIncrementalSegmentationSegmentCreateTurn.append(
                    ...
                    if len(listHypRefSegment) > 1:
                        ...
                        for y in range(2, len(listTmp)):
                            if modeNoGap == True:
                                if not (
                                    Segment.intersection(newSegment, listTmp[y]) is not None or newSegment["stop"] == listTmp[y]["start"]
                                    or newSegment["start"] == listTmp[y]["stop"]
                                ):
                                    logging.error("Cannot have absence of a segment in Transcriber mode.")
                                    raise Exception("Absence of a segment.")
...
def automatonSegmentationAssignment(
    diarHyp,
    diarRef,
    diarUem=None,
    tolerance=0,
    ...
    diarFinal__clusterToDeleteAccordingToDiarRef=list(),
    deleteBoundarySameConsecutiveSpk=False,
    deleteBoundaryMergeCluster=False
):
    assert isinstance(diarHyp,
                      Diar) and isinstance(diarRef, Diar) and isinstance(modeNoGap, bool) and (diarUem is None or isinstance(diarUem, Diar)) and isinstance(
                          tolerance, numbers.Number
                      ) and isinstance(diarFinal__clusterToDeleteAccordingToDiarRef,
        ...
        if len(listHypRefSegment) == 0 or (len(listHypRefSegment) == len(listHypRefSegmentWithinTolerance)):
            ...
            if modeNoGap == False:
                ...
                if segmentExistAccordingToTolerance(valueRef, tolerance):
                    ...
                    actionsSegmentationSegmentCreate.append(
                        copy.deepcopy(
                            Segment([valueRef['show'], valueRef['cluster'], valueRef['cluster_type'], valueRef['start'], valueRef['stop']],
                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                        )
                    )
                    actionsIncrementalSegmentationSegmentCreateTurn.append(
                    ...
                    if valueRef['cluster'] not in dictionary:
                        ...
                        actionsAssignmentCreate.append([
                            copy.deepcopy(valueRef['cluster']), 'speakerManualNotDetected' + str(cpt + 1),
                            Segment([valueRef['show'], valueRef['cluster'], valueRef['cluster_type'], valueRef['start'], valueRef['stop']],
                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                        ])
                        actionsIncrementalAssignmentCreateTurn.append([
                            copy.deepcopy(valueRef['cluster']), 'speakerManualNotDetected' + str(cpt + 1),
                            Segment([valueRef['show'], valueRef['cluster'], valueRef['cluster_type'], valueRef['start'], valueRef['stop']],
                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                        ])
                ...
                for idx, z in enumerate(listHypRefSegment):
                    ...
                    if valueRef['cluster'] not in dictionary:
                        if z['cluster'] in actionsAssignmentCreateBis:
                            ...
                            actionsIncrementalAssignmentCreateTurn.append([
                                copy.deepcopy(valueRef['cluster']), 'speakerManual' + str(cpt + 1),
                                copy.deepcopy(z)
                            ])
                            applyChange = True
                            cpt += 1
                        ...
                        for idx, z in enumerate(listHypRefSegment):
                            ...
                            if nearStop is not None and boundStop != valueRef['stop']:
                                if idx == 0 and z['start'] > valueRef['start'] and valueBoundaryStart != z['start']:
                                    actionsSegmentationSegmentCreate.append(
                                        copy.deepcopy(
                                            Segment([valueRef['show'], z['cluster'], z['cluster_type'], valueBoundaryStart, z['start']],
                                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                                        )
                                    )
                                    actionsSegmentationSegmentCreate.append(
                                        copy.deepcopy(
                                            Segment([valueRef['show'], z['cluster'], z['cluster_type'], z['stop'], nearStop],
                                                    ['show', 'cluster', 'cluster_type', 'start', 'stop'])
                                        )
                                    )
                                    actionsIncrementalSegmentationSegmentCreateTurn.append(
                                    ...
                                    valueTmp.append(
                                        show=showname, cluster=z['cluster'], cluster_type=z['cluster_type'], start=valueBoundaryStart, stop=z['start']
                                    )
...
def border(
    ...
    diarUem=None,
    tolerance=0,
    nonSpeechNameRef="NONSPEECHREF",
    nonSpeechNameHyp="NONSPEECHHYP",
    toleranceNameRef="TOLERANCE",
    toleranceNameHyp="TOLERANCE"
):
    assert isinstance(diarHyp, Diar) and isinstance(diarRef, Diar) and (diarUem is None or isinstance(diarUem, Diar)) and isinstance(
        nonSpeechNameRef, str
    ) and isinstance(nonSpeechNameHyp, str) and isinstance(tolerance, numbers.Number) and tolerance >= 0
    if diarUem is not None:
...
def borderHCIQ(
    ...
    assert isinstance(cb, numbers.Number) and isinstance(db, numbers.Number) and isinstance(cb, numbers.Number) and isinstance(
        cn, numbers.Number
    ) and isinstance(v, numbers.Number) and isinstance(sn, numbers.Number)
    assert cb >= 0 and db >= 0 and cn >= 0 and sn >= 0 and v >= 0
    get = borderHumanCorrectionsToDo(
        diarHyp, diarRef, diarUem, False, nameAlreadySet, tolerance, nonSpeechNameRef, nonSpeechNameHyp, toleranceNameRef, toleranceNameHyp
    )
...
def borderHumanCorrectionsToDo(
    diarHyp,
    diarRef,
    diarUem=None,
    verbose=False,
    ...
    assert isinstance(verbose, bool) and isinstance(diarHyp, Diar) and isinstance(diarRef,
                                                                                  Diar) and (diarUem is None
                                                                                             or isinstance(diarUem, Diar)) and isinstance(nameAlreadySet, set)
    if diarUem is not None:
        diarUem = compressDiar(diarUem)
    else:
        uem_start = min(diarRef.unique('start') + diarHyp.unique('start'))
        ...
        for idx, i in enumerate(listTmp):
            if idx == 0:
                ...
                if prev_hyp != prev_ref:
                    ...
                    if verbose:
                        logging.info(
                            prev_border, prev_idx, prev_l, 'ref=', prev_ref, 'hyp=', hyp, 'new hyp=', prev_hyp, 'cmp=', hyp == prev_ref, 'v=',
                            output["validate"], 'cb=', output["create_boundary"], 'db=', output["delete_boundary"], 'cn=', output["create_name"], 'sn=',
                            output["select_name"]
                        )
                else:
                    ...
                    if verbose:
                        logging.info(
                            prev_idx, ' ref=', prev_ref, ' hyp=', prev_hyp, ' cmp=', prev_ref == prev_hyp, 'v=', output["validate"], 'cb=',
                            output["create_boundary"], 'db=', output["delete_boundary"], 'cn=', output["create_name"], 'sn=', output["select_name"]
                        )
                if verbose:
                    logging.info('-' * 10)
            else:
                ...
                if verbose:
                    logging.info(
                        prev_border, prev_idx, prev_l, 'ref=', prev_ref, 'hyp=', hyp, 'new hyp=', prev_hyp, 'cmp=', hyp == prev_ref, 'v=', output["validate"],
                        'cb=', output["create_boundary"], 'db=', output["delete_boundary"], 'cn=', output["create_name"], 'sn=', output["select_name"], '\n'
                    )
    return output
...
def boundariesInTolerance(boundarySegment, segment, tolerance):
    ...
    if (boundarySegment['start'] <= segment['start'] + tolerance and boundarySegment['start'] >= segment['start'] - tolerance
        ) and (boundarySegment['stop'] <= segment['stop'] + tolerance and boundarySegment['stop'] >= segment['stop'] - tolerance):
        return True
    return False
...
def boundHypToChange(diarHyp, diarRef, diarUem=None, verbose=False, tolerance=25):
    assert isinstance(verbose, bool) and isinstance(diarHyp, Diar) and isinstance(diarRef,
                                                                                  Diar) and ((isinstance(diarUem, Diar) and len(diarOverlapArea(diarUem)) == 0)
                                                                                             or diarUem is None) and len(diarOverlapArea(diarRef)) == 0 and len(
                                                                                                 diarOverlapArea(diarHyp)
                                                                                             ) == 0
    diarHyp = copy.deepcopy(diarHyp)
    diarRef = copy.deepcopy(diarRef)
...
def compareDiar(diar1, diar2):
    ...
    for j, val in enumerate(diar1):
        if not (
            diar1[j]['show'] == diar2[j]['show'] and diar1[j]['cluster'] == diar2[j]['cluster'] and diar1[j]['cluster_type'] == diar2[j]['cluster_type']
            and diar1[j]['start'] == diar2[j]['start'] and diar1[j]['stop'] == diar2[j]['stop']
        ):
            return False
    return True
...
def countOccurenceSegment(segment, diar):
    ...
    cpt = 0
    for i in diar:
        if i['show'] == segment['show'] and i['cluster'] == segment['cluster'] and i['cluster_type'] == segment['cluster_type'] and i['start'] == segment[
            'start'] and i['stop'] == segment['stop']:
            cpt += 1
    return cpt
...
def cutBigSegmentLowEnergy(diar, cepstrum, maxSegSize, securityMarginSize, winSize=100):
    assert isinstance(diar, Diar) and isinstance(maxSegSize, numbers.Number) and isinstance(securityMarginSize,
                                                                                            numbers.Number) and isinstance(winSize, numbers.Number)
...
def existSegment(segment, diar):
    ...
    return (
        0 != len(
            diar.filter("show", '==', segment['show']).filter("cluster", "==", segment['cluster']).filter('cluster_type', "==", segment['cluster_type']).
            filter('start', "==", segment['start']).filter('stop', "==", segment['stop'])
        )
    )
...
def fillDiar(cluster, accordingToDiar, diar):
    assert isinstance(cluster, str) and isinstance(diar, Diar) and len(diar) != 0 and len(
        diar.unique('show')
    ) == 1 and isinstance(accordingToDiar, Diar) and accordingToDiar.duration() != 0
...
def FRFA(diarHyp, diarRef, diarUem=None, tolerance=25, cluster="fillFRFA"):
    assert isinstance(cluster, str) and (diarUem is None
                                         or isinstance(diarUem, Diar)) and isinstance(diarHyp,
                                                                                      Diar) and isinstance(diarRef,
                                                                                                           Diar) and isinstance(tolerance, numbers.Number)
...
def humanCorrectionsToDo(diarHyp, diarRef, diarUem=None):
    assert isinstance(diarHyp, Diar) and isinstance(diarRef, Diar) and (isinstance(diarUem, Diar) or diarUem is None) and len(
        diarOverlapArea(diarRef)
    ) == 0 and len(diarOverlapArea(diarHyp)) == 0
    diarHyp = copy.deepcopy(diarHyp)
    diarRef = copy.deepcopy(diarRef)
    if diarUem is not None:
        diarUem = copy.deepcopy(diarUem)
            ...
            if inter is not None:
                if i['cluster'] not in dictio:
                    ...
                    if y['cluster'] not in affected:
                        ...
                        diarCreateOut.append(
                            show=y['show'], cluster=y['cluster'], cluster_type=y['cluster_type'], start=y['start'], stop=y['stop'], matchCluster=i['cluster']
                        )
                    else:
...
def PurityCoverage(diarHyp, diarRef, diarUem=None, tolerance=25, skip_overlap=False):
    assert isinstance(skip_overlap,
                      bool) and (diarUem is None
                                 or isinstance(diarUem, Diar)) and isinstance(diarHyp, Diar) and isinstance(diarRef,
                                                                                                            Diar) and isinstance(tolerance, numbers.Number)
    tolerance = (tolerance / 100) * 2
...
def releaseFrames(start, stop, diar):
    ...
    out_diar = Diar()
    for row in diar:
        if (row['start'] >= segment['start'] and row['start'] < segment['stop']) or (row['stop'] > segment['start'] and row['stop'] <= segment['stop']):
            if (row['start'] >= segment['start'] and row['start'] < segment['stop']) and (row['stop'] > segment['start'] and row['stop'] <= segment['stop']):
                pass
...
def rmOvlpSegments(diar, epsilon):
    ...
    for i in range(1, len(diar) - 2):
        ...
        seg2 = diar[i + 1]
        seg3 = diar[i + 2]
        print(seg1['start'], seg2['start'], seg1['start'] == seg2['start'], seg1['stop'] == seg2['stop'], seg1.duration() <= epsilon)
...
def safeAnnotationDiar(diar, diarUem=None, addNoise=True, segmentFollowed=None, withoutSpeechTurn=100):
    assert isinstance(withoutSpeechTurn,
                      numbers.Number) and isinstance(diar,
                                                     Diar) and (isinstance(diarUem, Diar) or
                                                                diarUem is None) and (isinstance(segmentFollowed, numbers.Number)
                                                                                      or segmentFollowed is None) and isinstance(addNoise, bool)
    diar = copy.deepcopy(diar)
    if diarUem is None:
        diarUem = Diar()
...
def splitSegment(segment, diar, value):
    assert isinstance(segment, Segment) and isinstance(diar, Diar) and isinstance(value,
                                                                                  numbers.Number) and value > segment['start'] and value < segment['stop']
    out_diar = Diar()
    for i in diar:
        if i['show'] == segment['show'] and i['cluster'] == segment['cluster'] and i['cluster_type'] == segment['cluster_type'] and i['start'] == segment[
            'start'] and i['stop'] == segment['stop']:
