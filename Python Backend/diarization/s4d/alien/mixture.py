...
def sample_gaussian(mean, covar, cvtype='diag', n_samples=1):
    ...
    Returns
    -------
    observation : array, shape (n_features, n_samples)
...
class GMM(BaseEstimator):
    ...
    >>> numpy.random.seed(0)
    >>> observation = numpy.concatenate((numpy.random.randn(100, 1),
    ...
    >>> numpy.round(g.weights, 2)
    array([ 0.25,  0.75])
    >>> numpy.round(g.means, 2)
    array([[ 0.06],
           [ 9.94]])
    >>> numpy.round(g.covars, 2)
    ... #doctest: +NORMALIZE_WHITESPACE
    array([[[ 1.02]],
           [[ 0.96]]])
    >>> g.predict([[0], [2], [9], [10]])
    array([0, 0, 1, 1])
    >>> numpy.round(g.score([[0], [2], [9], [10]]), 2)
    ...
    def eval(self, obs):
        ...
        Compute the log probability of `observation` under the model and
        return the posterior distribution (responsibilities) of each
        mixture component for each element of `observation`.
    ...
    def decode(self, obs):
        """Find most likely mixture components for each point in `observation`.
        ...
        logprobs : array_like, shape (n_samples,)
            Log probability of each point in `observation` under the model.
    ...
    def rvs(self, n_samples=1):
        ...
        Returns
        -------
        observation : array_like, shape (n_samples, n_features)
            List of samples
        """
        ...
        for comp in range(self._n_states):
            # occurrences of current component in observation
...
def _covar_mstep_diag(gmm, obs, posteriors, avg_obs, norm, min_covar):
    # For column vectors:
    # covars_c = average((observation(t) - means_c) (observation(t) - means_c).T,
    #                    weights_c)
    # (observation(t) - means_c) (observation(t) - means_c).T
    #     = observation(t) observation(t).T - 2 observation(t) means_c.T + means_c means_c.T
    #
