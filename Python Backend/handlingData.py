import analyzingData as td
from collections import defaultdict

with open('SpeechToText') as f:
    lines = f.read().split('\n')
...
for i in range(len(chat)):
    ...
    f.write(chat[i])
    f.close()
    pred = td.test()  # Actual Prediction Result as a List
    # print(pred)
...
for i in range(len(chat)):
    ...
    sent[d[user[i]]].pop(0)
    print('\n\nUser', user[i], ':', chat[i], '\t\nSentiment:',
          list_user_sent[d[user[i]]][index])  # ,'\t\nSome words ',feedback[d[user[i]]][index]
    if list_user_sent[d[user[i]]][index] == 'Negative':
        print('Some words you could have used ', feedback[d[user[i]]][index])
